...
with app.app_context():
    ...
    for dirpath, dirnames, filenames in os.walk(folder_path):
        for filename in filenames:
            FilePrefix, FileExtension = os.path.splitext(filename)
            if ("CMT_Oraux_YYYY" in FilePrefix):
                j+=1
        for filename in filenames:
            FilePrefix, FileExtension = os.path.splitext(filename)
            if ("CMT_Oraux_YYYY" in FilePrefix):
                df = dico["CMT_Oraux_YYYY_"+select_filiere_oraux(FilePrefix)]
                tab = df.to_numpy()
                if(i != j):
                    i+=1
                    for row in tab:
...
with app.app_context():
    ...
    for dirpath, dirnames, filenames in os.walk(folder_path):
        for filename in filenames:
            FilePrefix, FileExtension = os.path.splitext(filename)
            if ("_CMT_spe_XXXX_SCEI" in FilePrefix):
                j+=1 # COMPTE LE NOMBRE DE FICHIERS DE CE TYPE DANS LE REPERTOIRE
        for filename in filenames:
            FilePrefix, FileExtension = os.path.splitext(filename)
            if ("_CMT_spe_XXXX_SCEI" in FilePrefix):
                df = dico["Classes_"+select_filiere(FilePrefix)+"_CMT_spe_XXXX_SCEI"]
                tab = df.to_numpy()
                if(i != j):
                    i+=1
                    for row in tab:
